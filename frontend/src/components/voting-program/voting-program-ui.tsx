...
function ProposalList() {
    ...
    // Helper function to properly display candidates
    const formatCandidates = (candidates: any) => {
        if (Array.isArray(candidates)) {
            return candidates.map(candidate => {
                // If candidate is an object, try to get a meaningful string representation
                if (typeof candidate === 'object' && candidate !== null) {
                    return candidate.id || candidate.name || candidate.toString();
                }
                return candidate;
            }).join(', ');
        }
            ...
            {proposals.length === 0 ? (
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {proposals.map((proposal, index) => (
                        <div
                            key={index}
                            className="bg-white dark:bg-neutral-800 rounded-lg shadow-md border border-neutral-200 dark:border-neutral-700 p-6 hover:shadow-lg transition-shadow duration-200"
                        >
                            {/* Proposal Title */}
                            ...
                            <div className="mb-4">
                                ...
                                <div className="mt-1 p-2 bg-neutral-0 dark:bg-neutral-700 rounded text-sm text-neutral-600 dark:text-neutral-300">
                                    {formatCandidates(proposal.data.candidates)}
                                </div>
                            </div>
                            ...
                            {/* Action Button (when you uncomment CastVote) */}
